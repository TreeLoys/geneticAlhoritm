...
from mainMigration import Evolution
...
class InitGuiVar():
    ...
    def initGuiVar(self):
        ...
        self.varUnderpopulationSize = IntVar()
        self.varUnderpopulationSize.set(self.settings.underpopulationSize)
        self.varUnderpopulationSize.trace_add("write", self.changeUnderpopulationSize)
    ...
    def changeUnderpopulationSize(self, v, i, m):
        self.settings.underpopulationSize = self.varUnderpopulationSize.get()
        print(self.settings.underpopulationSize)
        self.updateEvolution()
...
# Выбор количества подпопуляций
labelUnderpopulation = tk.Label(mutationFrame, text="Подпопуляций: ")
labelUnderpopulation.grid(column=2, row=0, pady=5)

eUnderpopulation = tk.Entry(mutationFrame, width=10, textvariable=s.varUnderpopulationSize)
eUnderpopulation.grid(column=3, row=0, pady=5)

# На каком шаге эволюция
label5 = tk.Label(mutationFrame, textvariable=s.varSliderEra)
label5.grid(column=4, row=0, pady=5)
